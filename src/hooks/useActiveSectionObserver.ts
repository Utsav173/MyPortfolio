...
export function useActiveSectionObserver(
  ...
  const handleActiveSectionChange = useCallback((entries: IntersectionObserverEntry[]) => {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(handleActiveSectionChange, options);
